...
const UpdateCoolerModal = ({ isOpen, onClose, coolerId, coolerName, onUpdate }) => {
            ...
            boxShadow: '0 1px 2px 0 black',
                ...
                <div className='space-y-7'>
                    <div className="mb-4">
                        ...
                        <input type="text" value={newCoolerId} 
    // readOnly
    onChange={(e) => setNewCoolerId(e.target.value)} className="mt-1 p-2 w-full border border-gray-300 rounded-md focus:outline-none focus:ring focus:border-blue-300"/>
                    </div>
                    <div className="mb-4">
                        <label className="block text-lg font-medium text-white text-start mb-3">Cooler Name</label>
                        <input type="text" value={newCoolerName} onChange={(e) => setNewCoolerName(e.target.value)} className="mt-1 p-2 w-full border border-gray-300 rounded-md focus:outline-none focus:ring focus:border-blue-300"/>
                    </div>
                </div>
...
const DeleteConfirmationModal = ({ isOpen, onClose, onDelete, coolerId, coolerName }) => {
    const handleDelete = () => {
        onDelete(coolerId);
        onClose();
    };
...
const ListCooler = () => {
    ...
    const [filteredCoolers, setFilteredCoolers] = useState(coolers);
    ...
    useEffect(() => {
        setFilteredCoolers(coolers.filter((cooler) => cooler.coolerName.toLowerCase().includes(filterText.toLowerCase())));
    }, [coolers, filterText]);
    useEffect(() => {
        const handleResize = () => {
    ...
    return (<>
            ...
            {!isLoading && (<div className="flex flex-col lg:flex-row xl:flex-col">
                    ...
                    <div className={`flex-1 p-4 lg:p-10 ${isSidebarOpen ? 'lg:ml-52 xl:ml-52' : ''}`}>
                        ...
                        <div className="flex flex-col items-center justify-center mt-2 overflow-y-auto">
                            ...
                            <UpdateCoolerModal isOpen={isUpdateModalOpen} onClose={() => setUpdateModalOpen(false)} coolerId={selectedCooler.coolerId} coolerName={selectedCooler.coolerName} onUpdate={handleUpdate}/>
                        </div>
                        ...
                        <div className="flex flex-col items-center justify-center mt-2 overflow-y-auto">
                            ...
                            <DeleteConfirmationModal isOpen={isDeleteModalOpen} onClose={() => setDeleteModalOpen(false)} onDelete={handleDeleteConfirm} coolerId={selectedDeleteCooler.coolerId} coolerName={selectedDeleteCooler.coolerName}/>
        ...
        value={filterText} onChange={(e) => setFilterText(e.target.value)}/>
                            ...
                            {filteredCoolers.length > 0 ? (<div className="text-black w-full overflow-x-auto">
                                    <table className="min-w-full bg-white border border-gray-300">
                                        ...
                                        <tbody>
                                            {filteredCoolers.map((cooler, index) => (<tr key={index} className="border-t font-medium">
