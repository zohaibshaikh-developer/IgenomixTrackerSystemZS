...
const CoolerOUT = () => {
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (response.status === 200) {
                ...
                navigate('/list-coolerOUT', { state: { message: response.data.message || 'Cooler OUT Data Inserted Successfully' } });
            }
            else {
                // Handle other status codes
                throw new Error(response.data.message || 'Failed to insert Cooler OUT data');
            }
        }
        catch (error) {
    ...
    return (<div className="flex items-start justify-start h-screen">
            ...
            <div className={`flex-1 p-10 ${isSidebarOpen ? 'lg:ml-44 xl:ml-44' : ''}`}>
                <div className="flex items-center justify-center fixed top-1.5 left-2 transform z-50">
                    ...
                    {(!isSidebarOpen || window.innerWidth >= 768) && (<div className={`ml-4 ${isSidebarOpen ? 'ml-28 xl:ml-52 lg:ml-52 md:ml-52' : 'ml-2'} text-black text-3xl mt-2`} style={{ fontFamily: 'Lugrasimo, cursive' }}>
                            Cooler OUT
                        </div>)}
                </div>
                ...
                <div className="flex flex-col items-center justify-center h-screen mt-[-5%]">
                    <div className="wrapper bg-white shadow-lg p-0 rounded-lg">
                        <div className="wrapper relative bg-cyan-400 shadow-lg p-10 rounded-lg" style={{
            ...
            backgroundRepeat: 'no-repeat',
                            ...
                            <div className="p-5 text-white z-10 relative">
                                <h1>Cooler OUT</h1>
                                <p className='mt-2'>Fill in the details below to add a new Cooler OUT entry.</p>
                            </div>
                        </div>
                        ...
                        <form className="p-6 space-y-9 min-w-full text-left">
                            ...
                            <div className="mb-4 relative min-w-full">
                                ...
                                <DatePicker selected={dateSent} onChange={(date) => setDateSent(date)} dateFormat="dd/MM/yyyy" className="w-full p-1.5 input-style" placeholderText="Select Date" required/>
                            </div>
                            ...
                            <div className="mb-4 relative min-w-full">
                                ...
                                <select className="w-full p-1.5 input-style" value={selectedClinic} onChange={(e) => setSelectedClinic(e.target.value)} required>
                                    ...
                                    {clinicList.map(clinic => (<option key={clinic.rowId} value={clinic.clinicName}>{clinic.clinicName}</option>))}
                                </select>
                            </div>
                            ...
                            <div className="mb-4 relative min-w-full">
                                ...
                                <select className="w-full p-1.5 input-style" value={selectedCooler} onChange={(e) => setSelectedCooler(e.target.value)} required>
                                    ...
                                    {coolerList.map(cooler => (<option key={cooler.rowId} value={cooler.coolerID}>{cooler.coolerID}</option>))}
                                </select>
                            </div>
