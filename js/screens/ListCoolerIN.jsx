...
const Alert = ({ message, onClose }) => {
    return (<div className="fixed top-16 md:top-4 lg:top-4 xl:top-4 left-1/2 transform -translate-x-1/2 bg-green-500 p-4 rounded-md shadow-md">
            <p className="text-white">{message}</p>
        </div>);
};
const UpdateModal = ({ isOpen, onClose, SrNo, onUpdate }) => {
    ...
    return (<div className={`fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 ${isOpen ? 'block' : 'hidden'}`}>
            <div className="p-6 rounded-xl shadow-md w-96" style={{ background: 'linear-gradient(135deg, #07a3b2, #a0f1ea)' }}>
                <div className="flex justify-end">
                    <button className="bg-transparent border-none p-0 mb-3" onClick={onClose}>
                        <FaTimes className="text-black rounded-full p-1 text-3xl hover:text-white" style={{
            ...
            boxShadow: '0 1px 2px 0 black',
                ...
                <form className="p-6 space-y-9 min-w-full text-left">
                    <div className="mb-4 relative min-w-full">
                        ...
                        <input type="text" className="w-full p-1.5 input-style" value={editableSrNo} onChange={(e) => setEditableSrNo(parseInt(e.target.value, 10))} required/>
                    </div>
                    <div className="mb-4 relative min-w-full">
                        <label className="text-black font-semibold block text-md mb-3">Date Sent</label>
                        <DatePicker selected={newDateSent} onChange={(date) => setNewDateSent(date)} dateFormat="dd/MM/yyyy" className="w-full p-1.5 input-style" placeholderText="Select Date" required/>
                    </div>
                    <div className="mb-4 relative min-w-full">
                        <label className="text-black font-semibold block text-md mb-3">coolerIN Name</label>
                        <select className="w-full p-1.5 input-style" value={selectedcoolerIN} onChange={(e) => setSelectedcoolerIN(e.target.value)} required>
                            <option value="" disabled>Select coolerIN</option>
                            {coolerINList.map((coolerIN) => (<option key={coolerIN.rowId} value={coolerIN.coolerINName}>
                    ...
                    <div className="mb-4 relative min-w-full">
                        ...
                        <select className="w-full p-1.5 input-style" value={selectedCooler} onChange={(e) => setSelectedCooler(e.target.value)} required>
                            <option value="" disabled>Select Cooler ID</option>
                            {coolerList.map((cooler) => (<option key={cooler.rowId} value={cooler.coolerID}>
                                    {cooler.coolerID}
                                </option>))}
                        </select>
                    </div>
                    <div className="flex items-center justify-center mt-8">
                        <button className={`btn-hoverFormSubmit color-1 relative ${submitting ? 'cursor-not-allowed opacity-75' : ''}`} type="button" onClick={handleSubmit} disabled={submitting}>
                            {submitting ? (<div className="absolute inset-0 flex items-center justify-center ">
                                    <svg className="animate-spin h-5 w-5 border-t-5 border-blue-200 rounded-full" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
const DeleteConfirmationModal = ({ isOpen, onClose, onDelete, SrNo, DateReceived, Quantity, coolerID }) => {
    const handleDelete = () => {
        onDelete(SrNo);
        onClose();
    };
...
const ListCoolerIN = () => {
    ...
    const [coolersIN, setcoolersIN] = useState([]);
    ...
    const [filteredcoolersIN, setFilteredcoolersIN] = useState(coolersIN);
    ...
    useEffect(() => {
        setFilteredcoolersIN(coolersIN.filter((coolerIN) => coolerIN.coolerID.includes(filterText)));
    ...
    return (<>
        ...
        // </div>
            ...
            {!isLoading && (<div className="flex flex-col lg:flex-row xl:flex-col">
                    {/* Sidebar */}
                    ...
                    <div className={`flex-1 p-4 lg:p-10 ${isSidebarOpen ? 'lg:ml-52 xl:ml-52' : ''}`}>
                        ...
                        <div className="flex flex-col items-center justify-center mt-2 overflow-y-auto">
                            ...
                            <UpdateModal isOpen={isUpdateModalOpen} onClose={() => setUpdateModalOpen(false)} SrNo={selectedcoolerIN.SrNo} DateSent={selectedcoolerIN.DateSent} coolerINName={selectedcoolerIN.coolerINName} coolerID={selectedcoolerIN.coolerID} onUpdate={handleUpdate}/>
                        ...
                        <div className="flex flex-col items-center justify-center mt-2 overflow-y-auto">
                            ...
                            <DeleteConfirmationModal isOpen={isDeleteModalOpen} onClose={() => setDeleteModalOpen(false)} onDelete={handleDeleteConfirm} SrNo={selectedDeletecoolerIN.SrNo} DateReceived={selectedDeletecoolerIN.DateReceived} Quantity={selectedDeletecoolerIN.Quantity} coolerID={selectedDeletecoolerIN.CoolerID}/>
                        ...
                        <div className="flex flex-col items-center justify-center mt-2 overflow-y-auto">
                            <form className="w-full mb-3 p-1">
                                <input className="form-control mb-4 w-full h-8 bg-cyan-500 p-2 font-semibold text-lg rounded-lg placeholder-white" type="text" placeholder="Search..." 
        // Use the filter function based on your requirements
        value={filterText} onChange={(e) => setFilterText(e.target.value)}/>
                            </form>

                            {filteredcoolersIN.length > 0 ? (<div className="text-black w-full overflow-x-auto">
                                    <table className="min-w-full bg-white border border-gray-300">
                                        ...
                                        <tbody>
                                            {filteredcoolersIN.map((coolerIN, index) => (<tr key={index} className="border-t font-medium">
                    ...
                    >
                        <FaPencilAlt className="text-black hover:text-cyan-500" />
                                                        ...
                                                        <button className="mr-2 bg-transparent border-none p-2" onClick={() => {
                        ...
                        setFocusElement(document.body); // Set focus to body after button click
                                ...
                                </div>) : (<div className="text-black text-center mt-4">No Data Found</div>)}
                        </div>
                    </div>
                </div>)}
        </>);
