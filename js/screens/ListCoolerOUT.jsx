            ...
            boxShadow: '0 1px 2px 0 black',
                ...
                <form className="p-6 space-y-9 min-w-full text-left">
                    ...
                    <div className="mb-4 relative min-w-full">
                        ...
                        <select className="w-full p-1.5 input-style" value={selectedClinic} onChange={(e) => setSelectedClinic(e.target.value)} required>
                            <option value="" disabled>Select Clinic</option>
                            {clinicList.map((clinic) => (<option key={clinic.rowId} value={clinic.clinicName}>
                                    {clinic.clinicName}
                                </option>))}
                        </select>
                    </div>
...
const DeleteConfirmationModal = ({ isOpen, onClose, onDelete, SrNo, DateSent, clinicName, coolerID }) => {
    const handleDelete = () => {
        onDelete(SrNo);
        onClose();
    };
...
const ListCoolerOUT = () => {
    ...
    useEffect(() => {
        setFilteredclinics(clinics.filter((clinic) => clinic.clinicName.toLowerCase().includes(filterText.toLowerCase())));
    }, [clinics, filterText]);
    useEffect(() => {
        const handleResize = () => {
    ...
    return (<>
            ...
            {!isLoading && (<div className="flex flex-col lg:flex-row xl:flex-col">
                    ...
                    <div className={`flex-1 p-4 lg:p-10 ${isSidebarOpen ? 'lg:ml-52 xl:ml-52' : ''}`}>
                        ...
                        <div className="flex flex-col items-center justify-center mt-2 overflow-y-auto">
                            ...
                            <UpdateModal isOpen={isUpdateModalOpen} onClose={() => setUpdateModalOpen(false)} SrNo={selectedclinic.SrNo} DateSent={selectedclinic.DateSent} clinicName={selectedclinic.clinicName} coolerID={selectedclinic.coolerID} onUpdate={handleUpdate}/>
                        ...
                        <div className="flex flex-col items-center justify-center mt-2 overflow-y-auto">
                            ...
                            <DeleteConfirmationModal isOpen={isDeleteModalOpen} onClose={() => setDeleteModalOpen(false)} onDelete={handleDeleteConfirm} SrNo={selectedDeleteclinic.SrNo} DateSent={selectedDeleteclinic.DateSent} clinicName={selectedDeleteclinic.clinicName} coolerID={selectedDeleteclinic.CoolerID}/>
        ...
        value={filterText} onChange={(e) => setFilterText(e.target.value)}/>
                            ...
                            {filteredclinics.length > 0 ? (<div className="text-black w-full overflow-x-auto">
                                    <table className="min-w-full bg-white border border-gray-300">
                                        ...
                                        <tbody>
                                            {filteredclinics.map((coolerOUT, index) => (<tr key={index} className="border-t font-medium">
