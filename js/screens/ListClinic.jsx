...
const UpdateclinicModal = ({ isOpen, onClose, SrNo, clinicName, onUpdate }) => {
            ...
            boxShadow: '0 1px 2px 0 black',
                ...
                <div className='space-y-7'>
                    <div className="mb-4">
                        ...
                        <input type="text" value={newSrNo} 
    // readOnly
    onChange={(e) => setNewSrNo(e.target.value)} className="mt-1 p-2 w-full border border-gray-300 rounded-md focus:outline-none focus:ring focus:border-blue-300"/>
                    </div>
                    <div className="mb-4">
                        <label className="block text-lg font-medium text-white text-start mb-3">clinic Name</label>
                        <input type="text" value={newclinicName} onChange={(e) => setNewclinicName(e.target.value)} className="mt-1 p-2 w-full border border-gray-300 rounded-md focus:outline-none focus:ring focus:border-blue-300"/>
                    </div>
                </div>
...
const DeleteConfirmationModal = ({ isOpen, onClose, onDelete, SrNo, clinicName }) => {
    const handleDelete = () => {
        onDelete(SrNo);
        onClose();
    };
...
const Listclinic = () => {
    ...
    const fetchDataListclinic = () => {
        axios.get(`${BASE_URL}/list-clinics`)
            .then((response) => {
    ...
    const handleDeleteConfirm = async (SrNo) => {
        try {
            ...
            if (response.data.status === 200) {
                setAlert({ message: `Clinic with Sr.No ${SrNo} deleted successfully.`, visible: true });
                setTimeout(() => {
                    setAlert({ message: '', visible: false });
                }, 5000);
                fetchDataListclinic();
            ...
            {!isLoading && (<div className="flex flex-col lg:flex-row xl:flex-col">
                    ...
                    <div className={`flex-1 p-4 lg:p-10 ${isSidebarOpen ? 'lg:ml-52 xl:ml-52' : ''}`}>
                        ...
                        <div className="flex flex-col items-center justify-center mt-2 overflow-y-auto">
                            ...
                            <UpdateclinicModal isOpen={isUpdateModalOpen} onClose={() => setUpdateModalOpen(false)} SrNo={selectedclinic.SrNo} clinicName={selectedclinic.clinicName} onUpdate={handleUpdate}/>
                        </div>
                        ...
                        <div className="flex flex-col items-center justify-center mt-2 overflow-y-auto">
                            ...
                            <DeleteConfirmationModal isOpen={isDeleteModalOpen} onClose={() => setDeleteModalOpen(false)} onDelete={handleDeleteConfirm} SrNo={selectedDeleteclinic.SrNo} clinicName={selectedDeleteclinic.clinicName}/>
        ...
        value={filterText} onChange={(e) => setFilterText(e.target.value)}/>
                            ...
                            {filteredclinics.length > 0 ? (<div className="text-black w-full overflow-x-auto">
                                    <table className="min-w-full bg-white border border-gray-300">
                                        ...
                                        <tbody>
                                            {filteredclinics.map((clinic, index) => (<tr key={index} className="border-t font-medium">
                        ...
                        handleEdit(clinic.Sr_No, clinic.clinicName);
